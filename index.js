...
async function handleRequest(request) {
  ...
  if(!path){
    ...
    const html= await fetch("https://crontab10.github.io/url-shortener/"+config.theme+"/index.html")
  ...
  if (location) {
    if (config.safe_browsing_api_key){
      if(!(await is_url_safe(location))){
        let warning_page = await fetch("https://crontab10.github.io/url-shortener/safe-browsing.html")
    ...
    if (config.no_ref=="on"){
      let no_ref= await fetch("https://crontab10.github.io/url-shortener/no-ref.html")
